...
export function validateTrunkInput(i: UpsertTrunkInput): void {
...
export function renderTrunksPjsipConfig(trunks: SipTrunk[] = listTrunks()): string {
  const lines: string[] = [];
...
export function renderTrunksDialplan(trunks: SipTrunk[] = listTrunks()): string {
  const lines: string[] = [];
  lines.push('; AUTO-GENERATED by Web (/trunks).');
  lines.push(`; updated_at: ${new Date().toISOString()}`);
  lines.push('');
